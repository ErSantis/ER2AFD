...
export function buildmDFAFromuDFA(
  ...
  nuevasTransicionesAFD: Map<string, Map<string, string>>,
  nuevosEstadosFinales: Set<string>,
  gruposEquivalentes: Map<string, string[]> // Nuevo retorno
} {
  ...
  const gruposEquivalentes = new Map<string, string[]>(); // Guardará los grupos de estados equivalentes
  ...
  agrupamientoEstados.forEach((estados, conjuntoSignificativo) => {
    ...
    // Guardar el grupo de estados equivalentes
    if (estados.length > 1) {
      gruposEquivalentes.set(estadoConservado, estados);
    }
  ...
  return { nuevasTransicionesAFD, nuevosEstadosFinales, gruposEquivalentes };
}
